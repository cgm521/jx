...
var popUpCal = {
    ...
    minDate: bdate, // The earliest selectable date, or null for no limit
    maxDate: null, // The latest selectable date, or null for no limit
    speed: "500", // Speed of display/closure
    ...
    selectDate: function(Input) {
        ...
        if (this.timeSeparators.length > 1) {
        	...
        	//赋值
        	//alert(Input.val());
        	$("#date1-button").children('span').text(Input.val());
        }
    },
    ...
    showCalendar: function() {
        ...
        var html = '<div id="title" >请选择日期</div>' +
        ...
        if (!this.changeYear) {
            ...
            html += '<div class="select"><span id="calendar_newYear" style="width:100%">';
            html += this.selectedYear+'&nbsp;年</span></div>';
        }
        if (!this.changeMonth) {
            html += this.monthNames[this.selectedMonth] + '&nbsp;';
        }
        else {
            html += '<div class="select"><span id="calendar_newMonth" style="width:100%;">';
            html += this.monthNames[this.selectedMonth];
            html += '</span></div>';
        }
        ...
        for (var dow = 0; dow < 7; dow++) {
            ...
            ((dow==0||dow==6)?' class="calendar_weekEndCell"':'')+
            	'>' + (this.changeFirstDay ? '<a>' : '') +
    ...
    adjustDate: function(offset, period, dontShow) {
    	var date;
        if (this.timeSeparators.length > 1) {
            date = new Date(this.selectedYear + (period == 'Y' ? offset : 0),
