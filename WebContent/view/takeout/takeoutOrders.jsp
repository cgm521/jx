...
function loadOrders(selectType,orderState){
	...
	if(isLoadhelf=='0' || isLoadcomp=='0'){
		...
		$.ajax({
			...
			type:"POST",
			dataType:"json",
			data:{"pk_group":pk_group,"openid":openid,"orderType":"2","orderState":orderState},
...
// 								+ "<div class='oneOrder'  style=\"border:1px solid #ddd;float:left;\"  id=\""+item.id+"\">"
								...
								+ "<tr>"
								+ "<td class='three-left'>"
								+ "<div style='color:#656565;font-size:80%;padding-bottom:5px;'>订单类型:&nbsp;外卖</div>"
...
function orderDetails(div,orderid,firmid){
	...
	window.setTimeout(function(){
		InitLayer();
		var url = "<c:url value='/takeout/orderDetail.do?pk_group=' />"+pk_group+"&openid="+openid
			+"&orderid="+orderid+"&firmid="+firmid+"&orderType=2&pageFrom=list&code=${code}";
								...
								<td class="three-right-menu">
									<c:choose>
										<c:when test="${o.state == 0}">
											<span class="redState">未提交</span>
										</c:when>
										<c:when test="${o.state == 1}">
											<c:choose>
												<c:when test="${o.payment eq '2'}">
													<span class="redState">未付款</span>
												</c:when>
												<c:otherwise>
													<span class="redState">已下单</span>
												</c:otherwise>
											</c:choose>
										</c:when>
