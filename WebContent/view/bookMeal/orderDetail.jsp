...
wx.ready(function () {
	...
	$(".scanToOrders").bind("click",function (){
		if(orderState=="1" || orderState=="2"){//其他状态的不能扫码下单
			//判断订单日期是否是当天，非当天订单不可下单
			var myDate = new Date();
			var nowy = myDate.getYear();
			var nowm = myDate.getMonth();
			var nowd = myDate.getDate();
			var resv = "${orderDetail.resv}";
			var ordery = resv.substring(0, 2);
			var orderm = resv.substring(2, 4);
			var orderd = resv.substring(4, 6);
			if((parseInt(nowm) != parseInt(orderm) - 1) || (parseInt(nowd) != parseInt(orderd))) {
				alertMsg("不是今日订单，请重新点菜下单！");
				return;
			}
			...
			//alertMsg("此单不支持扫码下单。");
			//return;
			var uri = "<%=path %>/evaluate/toEvaluate.do?openid=${openid}&pk_group=${pk_group}&ordersId=${orderDetail.id}";
...
// 	}
	...
	if("${hasAskCheckOut}" == "Y" || "${orderDetail.ordfrom}"=="HANDHELD") {
		// 如果已经点击过我要结帐，隐藏加菜按钮
		$("#addMenuBtn").hide();
...
// 	alert("openid=${openid},pk_group="+pk_group+",orderid="+ordersid+",firmCode="+firmCode+",pax="+pax);
	...
	$.post("<c:url value='/bookMeal/setScanEventValue.do?' />", {"openid":"${openid}", "pk_group":pk_group, "orderid":ordersid, "firmid":"${orderDetail.firmid}", "firmCode":firmCode, "pax":pax}, function(data){
		if(mustPayBeforeOrder == "Y" && orderState=="1"){//需要先支付才能再下单，调用支付界面
			wx.scanQRCode({
				needResult: 1,
				scanType: ["qrCode","barCode"],
				success: function (res) {
					var url = "<%=basePath %>/wxPay/toOrderPay.do?pk_group=${pk_group}&openid=${openid}&firmid=${orderDetail.firmid}&resv=${orderDetail.resv}&sumprice=${totalPrice}&pax=${orderDetail.pax}&id=${orderDetail.id}&response_type=code&scope=snsapi_base&state=123&code=${code}&openScan=1&billstate=2";
						window.setTimeout(function(){
							location.href = url;
					},500);
				}
	...
	if(mustPayBeforeOrder != "Y" || orderState!="1"){//关闭窗口
		window.setTimeout(function(){
			wx.closeWindow();
			closeLayer();
		},5000);
...
function toCheckout(){
	/*
	//如果是手持机上传的订单，点击我要结账直接进入支付页面
	if("${orderDetail.ordfrom}"=="HANDHELD"){
		InitLayer("正在跳转，请稍候。。。");
		var url = "<%=basePath %>/wxPay/toConfirmPage.do?id=${orderDetail.id}&firmid=${orderDetail.firmid}&resv=${orderDetail.resv}&code=${code}&billstate=6";
		window.setTimeout(function(){
			location.href = url;
		},100);
		return;
...
<div data-role="page" data-theme="d" id="pageone"><!--页面层容器-->
	<div data-role="content" style="margin-top:0;list-style:none;padding:0;"><!--页面主体-->
		<div id="headDiv" class="header scanToOrders">
			<img src="<c:url value='/image/wechat/laba.png'/>" /><span>您可以使用微信扫描桌上的二维码进行自助下单</span>
		</div>
		<div class="firm">
			<table width="90%" border="0" cellspacing="0" cellpadding="0" align="center" style="table-layout:fixed;">
				...
				<tr>
					...
					<td class="detail" style="width:30%;text-align: right;vertical-align: bottom;padding-top: 5px;padding-bottom: 0px;" rowspan="2">
						<span style="font-size:12px;">账单金额￥</span>
						<span style="font-family: Arial;font-size: 180%;color: red; padding-right:10px;">
							<c:choose>
								<c:when test="${orderDetail.paymoney ne null && orderDetail.paymoney ne '' && orderDetail.paymoney ne '0.00' }">
		...
		<div class="orderStateImg" style="margin-top: 105px;">
			...
			<table class="groupTable">
				...
				<tr>
					<td class="grouptd_left">
						<c:choose>
							<c:when test="${fn:contains(a.couponname,'美团')}">
								<img src="<c:url value='/image/wechat/meituan.jpg'/>"/>
							</c:when>
							<c:when test="${fn:contains(a.couponname,'糯米')}">
								<img src="<c:url value='/image/wechat/nuomi.jpg'/>"/>
							</c:when>
							<c:when test="${fn:contains(a.couponname,'大众')}">
								<img src="<c:url value='/image/wechat/dazhong.jpg'/>"/>
							</c:when>
							<c:otherwise>
								<img src="<c:url value='/image/wechat/tuangou.jpg'/>"/>
							</c:otherwise>
						</c:choose>
					</td>
