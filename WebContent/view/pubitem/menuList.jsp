			...
			$.post("<%=path %>/pubitem/listPackages.do?firmId="+firmid,protyp,function(data){
				...
				for(var i=0;i<data.length;i++){
					...
					if($("#list"+data[i].id).length > 0 ){//如果我的菜单里存在这个套餐了，显示的时候加上数量
						...
						$("#menu-table").append("<tr>"
						+"<td rowspan='2' onclick='taocan(this)' a='"+data[i].id+"' b='"+data[i].des+"' c='"+data[i].price+"' d='"+data[i].picsrc+"'><img src='"+data[i].picsrc+"' width='100px' height='70px'/></td>"
...
<div id="mymenu-main" style="margin:0 0 0 0;" class="main">
...
<%-- 	<c:if test="${orders.id ==null && orders.id =='' }"> --%>
<!-- 班禾需要 -->
		<div id="isstws" style="width:95%;height:auto;background:#f3f3f3;margin:0 auto;">
			<span>
				<select id='stws' name='stws'>
					<option value="0" selected="selected">是否外送</option>
...
<%-- 	</c:if> --%>
	<div id="bak" style="width:95%;height:auto;background:#f3f3f3;margin:0 auto;">
		<textarea name="remark" id="remark" style="color:red;margin:5px auto;" placeholder="如果您需要外送的话请输入外送地址，请在此留言..." maxlength="100"></textarea>
	</div>
	<!-- 选择支付方式 -->
...
<a href="<%=path %>/pubitem/getOrderMenus.do?firmid=${orders.firmid }&openid=${orders.openid }&dat=${orders.dat }" data-ajax="false" id="commitsuccessbtn"></a><!-- 点菜成功提示 -->
