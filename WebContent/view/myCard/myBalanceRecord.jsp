...
<script language="JavaScript" type="text/JavaScript">
	...
	function changeDate(dateStr) {
		...
		window.setTimeout(function(){
			window.location.href = "<%=path %>/myCard/listBalanceRecord.do?cardNo=${cardNo }&openid=${openid }&pk_group=${pk_group }&chargeDate=" + dateStr;
		},100);
	}
</script>
    	...
    	<div class="account" style="padding-bottom:0px;">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
				<tr style="background-color:#FFB400; height:60px;">
					...
					<td width="33%" class="headTxt">充值总额</br><span style="font-family:Arial;">${totalChargeAmt }</span></td>
					<td width="32%" class="headTxt">消费总额</br><span style="font-family:Arial;">${totalConsumeAmt }</span></td>
				</tr>
			</table>
		</div>
  	</div>
						...
						<div class="dataDiv"  data-ajax="false">
							<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
								...
								<tr height="30px">
									...
									<c:choose>
										<c:when test="${fn:indexOf(record.rmbamt, '-') < 0 && fn:indexOf(record.rmbamt, '+') >= 0 }">
											<td align="center" style="width:30%; font-size:120%; color:green;text-align: right;">
												<span style="font-family:Arial;margin-right: 15px;">${record.rmbamt }</span>
											</td>
										</c:when>
										<c:when test="${fn:indexOf(record.rmbamt, '-') < 0 && fn:indexOf(record.rmbamt, '+') < 0 }">
											<td align="center" style="width:30%; font-size:120%; color:green;text-align: right;">
										...
										<c:when test="${fn:indexOf(record.rmbamt, '-') >= 0 }">
											<td rowspan="2" align="center" style="width:20%; font-size:120%; color:red;text-align: right;">
												<span style="font-family:Arial;margin-right: 15px;">${record.rmbamt }</span>
											</td>
										</c:when>
									</c:choose>
