...
<script language="JavaScript" type="text/JavaScript">
	...
	function changeDate(dateStr) {
		...
		window.setTimeout(function(){
			window.location.href = "<%=path %>/myCard/listConsumeRecord.do?cardNo=${cardNo }&openid=${openid }&pk_group=${pk_group }&chargeDate=" + dateStr;
		},100);
	}
</script>
...
<div data-role="page" data-theme="d"><!--页面层容器-->
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
				<tr style="background-color:#FFB400; height:60px;">
					<td width="35%" class="headTxt">
						<a href="#overlay" data-rel="popup" id="selectDate" class="headTxt" style="border-right:0px;font-family:Arial;">${headDate }
					...
					<td width="65%" class="headTxt">消费总额</br><span style="font-family:Arial;">${totalAmt }</span></td>
				</tr>
			</table>
		</div>
  	</div>
	...
	<div data-role="content" style="margin-top:0px; padding-top:0px; background-color:#F7F7F7;"><!--页面主体-->
		...
		<div id="chargeRecord">
			<c:choose>
				<c:when test="${fn:length(listConsumeRecord) <= 0}">
					<div class="noData">未查询到消费记录</div>
				</c:when>
				<c:otherwise>
					<c:forEach items="${listConsumeRecord }" var="record">
						<div class="dataDiv"  data-ajax="false">
							<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
								...
								<tr height="30px">
									<td colspan="2" align="center" style="width:70%; font-size:100%;font-family:Arial;text-align: left;">
										&nbsp;&nbsp;&nbsp;${record.tim}
									</td>
									<c:if test="${fn:indexOf(record.amt, '-') < 0 }">
										<td align="center" style="width:30%; font-size:120%; color:green;text-align: right;">
											<span style="font-family:Arial;margin-right: 15px;">${record.amt }</span>
										</td>
									</c:if>
									<c:if test="${fn:indexOf(record.amt, '-') >= 0 }">
										<td align="center" style="width:30%; font-size:120%; color:red;text-align: right;">
											<span style="font-family:Arial;margin-right: 15px;">-${fn:replace(record.amt, '-', '') }</span>
										</td>
									</c:if>
								</tr>
							</table>
