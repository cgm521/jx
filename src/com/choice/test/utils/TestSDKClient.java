...
//	public static String softwareSerialNo="9SDK-EMY-0999-JCWRM";//软件序列号,请通过亿美销售人员获取
	//public static String key="123456";//序列号首次激活时自己设定
	
	public static String password="570456";// 密码,请通过亿美销售人员获取
...
//		Client client2=new Client(softwareSerialNo,password);
	...
	public static void testRegistEx() {
		int i = SingletonClient.getClient("3SDK-EBB-0130-JHUUN","106909991864").registEx(password);
	...
	public static void testSendSMS() {
		try {
			int i=SingletonClient.getClient("3SDK-EBB-0130-JHUUN","106909991864").sendSMS(new String[] {"15066695576"}, "过年好，新年快乐","",5);//带扩展码
	...
	public static int sendSMS(String tele, String content){
		try {
			String softwareSerialNo =Commons.softwareSerialNo;
			String key =Commons.key;
			int i=SingletonClient.getClient(softwareSerialNo,key).sendSMS(new String[] {tele}, content,5);//带扩展码
			System.out.println("testSendSMS====="+i);
			
			LogUtil.writeToTxt("SendSMSByEmay", "发送短信，手机号【" + tele + "】，短信内容【" + content + "】，发送结果【" + i + "】");
			
			return i;
		} catch (Exception e) {
			LogUtil.writeToTxt("SendSMSByEmay", "发送短信，手机号【" + tele + "】，短信内容【" + content + "】，发送结果【失败】，失败原因【" + e.getMessage() + "】");
			e.printStackTrace();
			return -1;
		}
	}
